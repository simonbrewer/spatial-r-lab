...
If we now check the list of available resamplers, you should see a few new ones with `spcv` in their names. All of these are different types of spatially robust resampling strategies. We'll use a basic blocking from Brenning et al. (2018) called `spcv_coords`. This is set up in a similar way as before using the `rsmp` function:
...
In this case the spatial resampling is slightly better than the original. This suggests that the covariates included in the model account for all of the spatial dependency. 
