...
Each unique string is now considered as a level in this factor grouping. If you now print the content of the vector by typing `penguins$species`, you should at the end the list of three levels (`setosa versicolor virginica`). Note that you can avoid this skip by reading in the data with the argument `stringsAsFactors = TRUE` in `read.csv()`
