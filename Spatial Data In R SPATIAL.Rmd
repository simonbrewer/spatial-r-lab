...
R can use many different file types, but comma-separated value (csv) files are recommended as the easiest way to transfer between R and Excel. Start downloading the file *iris.csv* to your computer, then change R's working directory to where this file is located. Then get a list of files as follows (note the use of the `pattern` parameter to filter the list of files returned):
...
This will write a csv file called _test.csv_ in your working directory, containing the values in the vector `plpw`, created from the original data frame . Note that R uses the variable name as a column header and adds a column with line numbers. You can remove the line numbers by adding the parameter `row.names=FALSE` to the `write.csv()` function. 
...
If you check the data types for each of these new vectors using `class()` you should see that the first two are numeric and the third is a vector of character strings (the species names).  R has a additional data type called *factors* which can be used to regroup your data (we'll see examples of this later). We'll convert the vector of species names to factors using `as.factor()`:
...
Each unique string is now considered as a level in this factor grouping. If you now print the content of the vector by typing `sp`, you should at the end the list of three levels (`setosa versicolor virginica`). 
...
Try calculating the standard deviation for each species.

### Some other useful functions:

`levels()` and `table()` are very useful for summarizing factor data (`table()` can also be used to make cross tables). The `summary()` function is one of the most important as it is used frequently in R to print output from analysis as well as summary statistics.
...
R has a series of add-on packages designed for use with spatial data. If you are using your own computer, you will need to install these before proceeding (these should be preinstalled on the lab computers). To install a package, either use the `Install` button from the 'Packages' tab, or use the following command, where *nameofmypackage* is the package you wish to install:
...
Install these now if you need to (note that you can include the name of multiple packages in the `install.packages()` command). Once installed, you will need to load each package into R's workspace in order to use it:

```{r eval=FALSE}

library(nameofmypackage)

```

**Packages only need to be installed once, but need to be loaded every time you restart R.**
...
The zip file *UT_WI2.zip* contains a shapefile with water isotope data recorded from a set of locations in Utah. In order to read this in to R, we first load the required packages, then read in the shapefile data, using the function `st_read()`. This takes a single argument: the name of the shapefile to load (including any directory information):

```{r results='hide', message=FALSE}

library(sf)
ut_wi = st_read("./UT_WI2/ut_wi2.shp")
class(ut_wi)

```

The data is read into a `sf` object, which is a data frame. This include an additional column called `geometry` which contains the coordinate information for each observation. 
...
ut_wi$WI_Analy_2 <- as.factor(ut_wi$WI_Analy_2)
plot(ut_wi["WI_Analy_2"], pch = 16)

```

## Polygon objects

Polygons can be created using a combination of the `st_polygon()` function but by far the easiest way to get polygon data into R is, again, to import it as a shapefile. The zip file *utahcounty.zip* contains a shapefile with county boundaries for Utah as polygons. Unzip this in your current working directory, then read in the data as follows:

```{r message=FALSE}

ut_cnty <- st_read("utahcounty/utahcounty.shp")
st_crs(ut_cnty) <- 4326
class(ut_cnty)

```

We can see the names of the variables in the attribute table as follows, and extract the list of county names:
...
We can again make a simple plot of the county boundaries. We can add the location of the isotope samples by using the `add=TRUE` parameter:

```{r fig.keep='none'}

plot(st_geometry(ut_cnty))
plot(st_geometry(ut_wi), col=2, pch=16, add=TRUE)
...
library(stars)
win_d2h <- read_stars("predkrig2.asc")
st_crs(win_d2h) <- 4326
class(win_d2h)
...
slc_wi = ut_wi[slc,]
plot(st_geometry(slc))
plot(st_geometry(slc_wi), col=3, add=TRUE)

```

We can also crop out the part of the IsoMAP grid for this county. 
...
The `st_extract()` function does overlay calculations. For example, we can get the value of the gridded data at each of the SPATIAL point locations as follows:
...
mybbox = st_bbox(slc)
grid = st_as_stars(mybbox, dx = 0.001, dy = 0.001)
st_crs(grid) = 4326
grid = grid[slc, ]
...
The next step is to fit a variogram model to capture this spatial structure. To start, we fit a model by eye. We need to specify: a) the model form; b) the value of the nugget (the intercept with the Y axis); c) the model range (the distance at which the sample variogram becomes flat); d) the sill, the semivariance value (y-axis) of the range. Here we specify these as separate variables, then use the `vgm()` function to build our model. 
